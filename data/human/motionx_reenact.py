...
class MotionX_ReEnact(object):
    def __init__(self, root: str = MOTIONX_REENACT_ROOT, from_zip: bool = True) -> None:
        ...
        if from_zip:
            ...
            for filepath in archive.namelist():
                ...
                elif filepath.startswith("inpainting_p3m_seg/") and filepath.endswith(
                    "mp4"
                ):
                    filekey = osp.splitext(
                        filepath.replace("inpainting_p3m_seg/", "", 1)
                    )[0]
    ...
    def parse_camera_params(self, camera_params) -> dict:
        ...
        extrinsic = (
            np.eye(4).reshape(1, 4, 4).repeat(camera_params["cam_R"].shape[0], axis=0)
        )  # [N, 4, 4]
        ...
        fx, fy = (
            camera_params["intrins"][:, 0],
            camera_params["intrins"][:, 1],
        )  # [N], [N]
        cx, cy = (
            camera_params["intrins"][:, 2],
            camera_params["intrins"][:, 3],
        )  # [N], [N]
    ...
    def load_json_params(self, filepath: str):
        ...
        new_smplx_params = {
            "global_orient": smplx_params[
                "root_orient"
            ],  # controls the global root orientation, [N, 3]
            "body_pose": smplx_params["pose_body"],  # controls the body, [N, 63]
            "left_hand_pose": smplx_params["pose_hand"][
                :, :45
            ],  # controls the finger articulation, [N, 45]
            "right_hand_pose": smplx_params["pose_hand"][
                :, 45:
            ],  # controls the finger articulation, [N, 45]
            ...
            "transl": smplx_params[
                "trans"
            ],  # controls the global body position, [N, 3]
    ...
    def overlay_pngs_on_video(
        ...
        try:
            from dw_utils.video import VideoWriterPyAV
        ...
        while True:
            ...
            if png_img.shape[2] == 3:
                ...
                png_img = np.dstack(
                    (
                        png_img,
                        np.full(
                            (png_img.shape[0], png_img.shape[1]), 255, dtype=np.uint8
                        ),
                    )
                )
            ...
            for c in range(3):
                resized_frame[:, :, c] = (
                    alpha_channel * color_channels[:, :, c]
                    + (1 - alpha_channel) * resized_frame[:, :, c]
                )

            if save_iamges:
                cv2.imwrite(
                    osp.join(out_overlay_folder, f"{frame_count:06d}.png"),
                    resized_frame,
                )
                if save_video_frames:
                    ...
                    cv2.imwrite(
                        osp.join(out_video_folder, f"{frame_count:06d}.png"), raw_frame
                    )
