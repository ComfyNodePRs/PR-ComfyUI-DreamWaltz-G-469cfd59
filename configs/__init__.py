...
class NeRFConfig:
    ...
    bg_mode: str = (
        "gray"  # {'nerf', 'random', 'white', 'black',  'gray', 'gaussian', 'zero'}
    )
...
class PromptConfig:
    ...
    canonical_pose: str = (
        "canonical-A-adjust"  # 'canonical', 'canonical-A', 'canonical-T'
    )
...
class TrainConfig:
    ...
    sigma_guidance_peak: float = 15.0
    ...
    def __post_init__(self):
        ...
        if self.log.pretrain_only and self.guide.controlnet_condition[0] != "depth_raw":
            logger.warning(
                f"NOTICE! log.pretrain_only=True, but guide.controlnet_condition = {self.guide.controlnet_condition} -> Manually setting guide.controlnet_condition to depth_raw"
            )
        ...
        if self.log.nerf2gs:
            if self.stage != "gs":
                logger.warning(
                    'NOTICE! log.nerf2gs = True, but stage != "gs". Manually setting stage to "gs".'
                )
                self.stage = "gs"
