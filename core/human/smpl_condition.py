...
from dw_utils.trimesh import render_trimesh, to_trimesh
from dw_utils.point3d import *
from dw_utils.mesh import export_normal_nvdiffrast
from dw_utils.se3 import SE3_Mat2RT
from data.camera.utils import adjust_intrinsics_size
from dw_utils import open3d as o3d_utils


def to_controlnet_pose(
    predicts, intrinsics, include_hand=True, include_face=True, distances=None
):
    """
...
class OcclusionCulling:
    def __init__(
        self, smpl_type: str, ignore_body_self_occlusion: bool = False
    ) -> None:
        if smpl_type == "smpl":
            ...
            self.face_indices = [0, 14, 15, 16, 17] + [
                i for i in range(18 + 21 * 2, 128)
            ]
            self.hand_indices = [i for i in range(18, 18 + 21 * 2)]
            self.body_indices = [
                i
                for i in range(128)
                if (i not in self.face_indices) and (i not in self.hand_indices)
            ]
        else:
    ...
    def __call__(
        ...
        directions /= np.linalg.norm(
            directions, ord=2, axis=2, keepdims=True
        )  # [N, K, 3]
        ...
        # Face Occlusion
        occluded_face = (
            t_far[:, self.face_indices] - t_hit[:, self.face_indices]
        ) > thres_face  # [N, 5], bool

        # Hand Occlusion
        occluded_hand = (
            t_far[:, self.hand_indices] - t_hit[:, self.hand_indices]
        ) > thres_hand  # [N, 42], bool

        # Body Occlusion
        occluded_body = (
            t_far[:, self.body_indices] - t_hit[:, self.body_indices]
        ) > thres_body  # [N, 13], bool
        if self.ignore_body_self_occlusion:
            self_geometry_ids = np.array(
                [[i for i in range(len(keypoints))]]
            ).T  # [N, 1]
            self_occluded_body = (
                geometry_ids[:, self.body_indices] == self_geometry_ids
            )  # [N, 13], bool
...
class SMPL2Condition:
    def __init__(self, cfg: PromptConfig) -> None:
        ...
        if cfg.use_occlusion_culling:
            self.occlusion_culling = OcclusionCulling(
                cfg.smpl_type, cfg.ignore_body_self_occlusion
            )
        else:
    ...
    def export_2d_keypoints(self, keypoints, **camera_params):
        ...
        keypoints_cam = transform_keypoints_to_novelview(
            keypoints.reshape(-1, 3), None, None, R, T
        )
    ...
    def export_pose(self, keypoints, ray_casting_scene, **camera_params):
        ...
        if self.occlusion_culling is not None:
            ...
            distances = None
        # Draw
        controlnet_pose = to_controlnet_pose(
            keypoints_img, intrinsics=intrinsics, distances=distances
        )
        image = draw_poses(
    ...
    def export_depth(
        ...
        **camera_params
        ...
        depth = o3d_utils.cast_rays(
            ray_casting_scene, return_type="depth", **camera_params
        )
        if raw:
    ...
    def export_normal(self, vertices, triangles, mvp, h, w):
        ...
        normal = export_normal_nvdiffrast(
            vertices, triangles, mvp=mvp, h=h, w=w, glctx=self.glctx, same_scene=True
        )
    ...
    def export_normal_raw(self, ray_casting_scene, raw=False, **camera_params):
        normal = o3d_utils.cast_rays(
            ray_casting_scene, return_type="normal", **camera_params
        )
        if raw:
    ...
    def __call__(
        ...
        # Numpy Style
            ...
            assert intrinsics.ndim == 2 and intrinsics.size == 9
            intrinsics = adjust_intrinsics_size(
                intrinsics=intrinsics, width=condition_width, height=condition_height
            )
            camera_params = {
                "intrinsics": intrinsics,
                "extrinsic": extrinsic,
            ...
            if condition_type in ("pose", "openpose"):
                ...
                ray_casting_scene = o3d_utils.build_ray_casting_scene(
                    vertices, triangles
                )
