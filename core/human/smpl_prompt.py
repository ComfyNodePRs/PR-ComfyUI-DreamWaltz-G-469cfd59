...
class SMPLPrompt:
    ...
    def __init__(
        self,
        cfg: PromptConfig,
        smpl_model: SemanticSMPLModel,
        cond_type: Union[Iterable, str],
        height: int,
        width: int,
        _dataset: Any = None,
    ):
        ...
        self.canonical_betas = parse_betas(
            cfg.canonical_betas, num_betas=num_betas, device=self.device
        )
        self.observed_betas = parse_betas(
            cfg.observed_betas, num_betas=num_betas, device=self.device
        )
    ...
    def __call__(
        self, frame_idx: Optional[int] = None, batch_idx: Optional[int] = None
    ):

        extra_smpl_inputs = {}
        if self.observed_betas is not None:
            betas = sample_betas(
                self.observed_betas, i=batch_idx, max_iteration=self.max_beta_iteration
            )
                    ...
                    if smpl_outputs is None:
                        ...
                        for k in smpl_outputs.keys():
                            if smpl_outputs[k] is not None:
                                smpl_outputs[k] = torch.cat(
                                    (smpl_outputs[k], _smpl_outputs[k]), dim=0
                                )
    ...
    def write_video(
        ...
        **kwargs,
        ...
        from dw_utils.video import VideoWriterPyAV as VideoWriter
        ...
        if cond_type is None:
            cond_type = (
                self.cond_type[0]
                if isinstance(self.cond_type, Iterable)
                else self.cond_type
            )
