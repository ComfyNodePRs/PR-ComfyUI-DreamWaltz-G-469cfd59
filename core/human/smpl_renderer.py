...
from dw_utils.mesh import Mesh, MeshRenderer, vertex_colors_to_albedo_image
...
class SMPLRenderer(nn.Module):
    def __init__(
        self, smpl_model: SMPLModel, body_mapping=None, albedo="smplx", albedo_res=1024
    ) -> None:
        super().__init__()
        ...
        # init albedo
        albedo = self.get_init_albedo(
            albedo, albedo_res, body_mapping=body_mapping, mesh=mesh
        )
    ...
    def get_init_albedo(self, albedo, albedo_res, body_mapping=None, mesh=None):
        if albedo == "zeros":
            ...
            skin_vids, skin_color = (
                body_mapping["skin"]["vertex_indices"],
                body_mapping["skin"]["rgb"],
            )
            eyes_vids, eyes_color = (
                body_mapping["eyes"]["vertex_indices"],
                body_mapping["eyes"]["rgb"],
            )
    ...
    def render(
        self, smpl_outputs: SMPLXOutput, data: dict, mesh_only=True, shading="albedo"
    ):
        """
        Returns:
