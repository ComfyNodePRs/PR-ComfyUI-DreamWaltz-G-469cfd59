...
class _Visualizer:
    ...
    def snapshot_image(
        ...
        save_path = (
            self.snapshot_train_path
            / filename
            / f"{filename}_{self.train_step:05d}.{extname}"
        )
    ...
    def concat_alpha(image: torch.Tensor, alpha: torch.Tensor, color_channel: int = 3):
        ...
        if image.shape[1:3] != alpha.shape[1:3]:
            ...
            alpha = F.interpolate(
                alpha, size=image.shape[1:3], mode="bilinear", align_corners=False
            )
    ...
    def snapshot_feature(
        ...
        if save_raw:
            save_path = (
                self.snapshot_train_path
                / f"{filename}_raw"
                / f"{filename}_{self.train_step:05d}.pt"
            )
            save_path.parent.mkdir(exist_ok=True)
    ...
    def snapshot_condition(
        ...
        if isinstance(image, list) and isinstance(filename, List):
            ...
            save_path = (
                self.snapshot_train_path
                / dirname
                / f"{filename}_{self.train_step:05d}.png"
            )
            save_path.parent.mkdir(exist_ok=True)
        ...
        for var in ("image", "image_fg", "image_gradients"):
            ...
            if var in render_outputs:
                if var == "image_gradients":
                    ...
                    grad = (
                        render_outputs[var] / (render_outputs[var].std() + 1e-9)
                    ) / 2.0
                    ...
                    if image.size(kwargs["color_channel"]) == 4:
                        image = self.latent_to_image(
                            image, color_channel=kwargs["color_channel"]
                        )
                    if var == "image_fg":
                        image = self.concat_alpha(
                            image, alpha=render_outputs["alpha"], color_channel=3
                        )
            ...
            if var in sd_outputs:
                ...
                if var == "gradients":
                    self.snapshot_feature(
                        sd_outputs[var], **kwargs, is_latent=True
                    )  # save_raw=self.train_step in (1, 100, 300, 500)
                else:
                    self.snapshot_latent(sd_outputs[var], **kwargs)

        if "cond_images" in data:
            self.snapshot_condition(
                data["cond_images"], filename=self.controlnet_condition
            )
...
class _Checkpointer:
    ...
    def load_checkpoint(self, checkpoint, model_only=False):
        if model_only:
            ...
            logger.info(
                f"Load model weights and optimizer checkpoints from {checkpoint}..."
            )
        ...
        if (
            hasattr(self, "optimizers")
            and self.optimizers is not None
            and "optimizers" in checkpoint_dict
        ):
            try:
                for optimizer, state_dict in zip(
                    self.optimizers, checkpoint_dict["optimizer"]
                ):
...
class _Logger:
    ...
    def time_to_evaluate(self) -> bool:
        return (
            self.train_step % self.cfg.log.evaluate_interval == 0
            or self.train_step in (1, 250)
        )

    @property
    def time_to_snapshot(self) -> bool:
        return (
            self.train_step % self.cfg.log.snapshot_interval == 0
            or self.train_step in (1, 100, 200, 300, 400)
        )
...
class Trainer(_Visualizer, _Checkpointer, _Logger):
    def __init__(self, cfg: TrainConfig):
        ...
        if self.cfg.stage == "nerf":
            ...
            self.smpl_renderer = SMPLRenderer(
                smpl_model=self.smpl_model, body_mapping=None, albedo="smplx"
            )
        else:
        ...
        if not self.cfg.log.eval_only:
            ...
            # Init Solvers
            if self.cfg.stage == "nerf":
                self.optimizers, self.schedulers, self.densifiers = (
                    self.init_nerf_solvers()
                )
            elif self.cfg.stage == "gs":
                self.optimizers, self.schedulers, self.densifiers = (
                    self.init_gaussian_solvers()
                )
    ...
    def export_samples(self, save_dir, num_samples=2):
        ...
        # SD
            ...
            isinstance(self.diffusion, BasicScoreDistillation) and self.cfg.log.check_sd
        ):
    ...
    def init_diffusion(self):
        if not self.cfg.guide.use_sdxl:
            ...
            from core.guidance.stable_diffusion import (
                ScoreDistillationXL as ScoreDistillation,
            )
            from core.guidance.controlnet import (
                ControlNetScoreDistillationXL as ControlNetScoreDistillation,
            )
    ...
    def init_nerf_model(self):
        ...
        logger.info(
            f"Loaded {self.cfg.nerf.backbone} NeRF, #parameters: {sum([p.numel() for p in model.parameters() if p.requires_grad])}"
        )
        # logger.info(model)
    ...
    def init_gaussian_model(self):
        ...
        if self.cfg.character is not None:
            ...
            for v in outfit_mapping.values():
                v["vertex_indices"], v["face_indices"] = (
                    self.smpl_model.get_semantic_indices(v["parts"])
                )
        ...
        if self.cfg.render.from_nerf is not None:
            from core.nerf.nerf_model import build_NeRFNetwork
            from core.nerf.to_point_cloud import (
                export_point_cloud,
                remove_points_inside_bboxes,
            )
            ...
            point_cloud = export_point_cloud(
                nerf, resolution=self.cfg.render.nerf_resolution, split_size=256
            )
            if self.cfg.render.nerf_exclusion_bboxes is not None:
                n_points = len(point_cloud.points)
                point_cloud = remove_points_inside_bboxes(
                    point_cloud, bboxes=eval(self.cfg.render.nerf_exclusion_bboxes)
                )
                logger.info(
                    f"Remove {n_points - len(point_cloud.points)} points inside bboxes: {self.cfg.render.nerf_exclusion_bboxes}"
                )
            logger.info(
                f"Load {self.cfg.nerf.backbone} NeRF and convert to 3d point cloud."
            )
        else:
            nerf = None
            point_cloud = None
        ...
        logger.info(
            f"Initialize Scene Model with #Parameters: {sum([p.numel() for p in model.parameters() if p.requires_grad])}"
        )
        logger.info(avatar)
    ...
    def init_gaussian_solvers(self):
        ...
        if self.cfg.render.use_densifier:
            solvers["densifiers"] = {}
            avatar_densifier = self.model.avatar.get_densifier(
                cfg=self.cfg, optimizer=solvers["optimizers"]["avatar"]
            )
    ...
    def init_dataloaders(self) -> Dict[str, DataLoader]:
        ...
        if self.smpl_prompt is not None:
            _CameraDataset = partial(
                CameraDatasetWithSMPL, **dataset_kwargs, smpl_prompt=self.smpl_prompt
            )
        else:
            _CameraDataset = partial(CameraDataset, **dataset_kwargs)
        # Build Training Set
        train_h, train_w = eval(str(self.cfg.data.train_h)), eval(
            str(self.cfg.data.train_w)
        )
        train_h = (train_h,) if isinstance(train_h, Number) else train_h
        train_w = (train_w,) if isinstance(train_w, Number) else train_w
        train_dataloaders = [
            _CameraDataset(
                mode="train", H=H, W=W, eval_size=self.cfg.optim.iters
            ).dataloader(batch_size=self.cfg.optim.batch_size)
            for H, W in zip(train_h, train_w)
        ]
        # Build Val and Test Sets
        eval_h, eval_w = self.cfg.data.eval_h, self.cfg.data.eval_w
        val_loader = _CameraDataset(
            mode="val", H=eval_h, W=eval_w, eval_size=self.cfg.data.eval_size
        ).dataloader()
        test_h, test_w = self.cfg.data.test_h, self.cfg.data.test_w
        test_loader = _CameraDataset(
            mode="test", H=test_h, W=test_w, eval_size=self.cfg.data.full_eval_size
        ).dataloader()
        # Return
        ...
        if self.cfg.stage == "nerf":
            if self.model.training:
                if self.cfg.guide.diffusion != "normal-adapted":
                    render_outputs = self.model.render(
                        data=data, shading="albedo", bg_mode=bg_mode
                    )
                else:
                    render_outputs = self.model.render(
                        data=data, shading="normal", bg_mode=bg_mode
                    )
            else:
                render_outputs = self.model.render(
                    data=data, shading="albedo", bg_mode=bg_mode
                )
                render_outputs["normal"] = self.model.render(
                    data=data, shading="normal", bg_mode=bg_mode
                )["image"]
    ...
    def calc_sigma_loss(
        self, data, render_outputs, sd_inputs, selected_parts, wo_wrist: bool = True
    ):
        ...
        from dw_utils.trimesh import to_trimesh
        ...
        distances_pow2, closest_fids, _ = point_mesh_squared_distance(
            noisy_points, mesh.vertices, mesh.faces
        )  # np.ndarray (N,)
        ...
        if wo_wrist:
            wrist_fids = self.smpl_model.get_semantic_indices(select_parts=["wrists"])[
                1
            ]
        ...
        xyzs = torch.from_numpy(np.concatenate((points, new_points), axis=0)).to(
            sd_inputs
        )
        ...
        if loss_type.startswith("opacity"):
            ...
            opacities_gt = torch.cat(
                (torch.ones(points.shape[0]), torch.zeros(new_points.shape[0]))
            ).to(sd_inputs)
        ...
        if self.time_to_snapshot:
            ...
            _std = _sigmas.std().item()
            logger.info(
                f"{selected_parts}_mesh_sigmas: min={_min:.2f}, max={_max:.2f}, std={_std:.2f}"
            )
            _sigmas = sigmas[num_samples:].detach()
            _min = _sigmas.min().item()
            _max = _sigmas.max().item()
            _std = _sigmas.std().item()
            logger.info(
                f"{selected_parts}_empty_sigmas: min={_min:.2f}, max={_max:.2f}, std={_std:.2f}"
            )
    ...
    def train(self):
        ...
        data_manager = DataLoaderManager(
            ...
            hook_func=lambda iterator, train_step, max_step: iterator.loader._data.set_training_ratio(
                train_step, max_step
            ),
        )
        ...
        try:
            while self.train_step < self.max_step:
                ...
                if self.cfg.stage == "nerf":
                    if (
                        self.model.cuda_ray
                        and self.train_step % self.cfg.nerf.update_extra_interval == 0
                    ):
                        with torch.cuda.amp.autocast(enabled=self.cfg.optim.fp16):
                ...
                for optimizer in self.optimizers.values():
                    ...
                    if hasattr(optimizer, "update_learning_rate"):
                        optimizer.update_learning_rate(
                            iteration=self.train_step, spatial_scale=spatial_scale
                        )
                ...
                if self.time_to_evaluate:
                    logger.info(
                        f"Evaluating and saving model, iteration #{self.train_step}..."
                    )
                ...
                if self.time_to_snapshot:
                    ...
                    logger.info(
                        f"iter={self.train_step:06d}, t={t:04d}, text={text}, azim={azim:.1f}, elev={elev:.1f}, scale={spatial_scale:.2f}"
                    )
    ...
    def train_forward(self, data: Dict[str, Any]):
        ...
        if self.cfg.guide.grad_rgb_clip_mask_guidance:
            if self.cfg.stage == "nerf":
                sd_kwargs["mask_inputs"] = (
                    render_outputs["weights_sum"].permute(0, 3, 1, 2).contiguous()
                )
            else:
                assert 0
        ...
        if self.cfg.stage == "nerf":
            ...
            elif (self.cfg.use_sigma_hand_guidance and "hand" in data["body_part"]) or (
            ...
            if sigma_select_parts is not None:
                regularizations.update(
                    self.calc_sigma_loss(
                        data,
                        render_outputs,
                        sd_inputs,
                        selected_parts=sigma_select_parts,
                    )
                )
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        gif_writers = {}
        if save_as_video:
            from dw_utils.video import VideoWriterPyAV, VideoWriterPIL

            for output_type in output_types_for_video:
                video_filename = (
                    output_type
                    if video_name is None
                    else output_type + "_" + video_name
                )
                video_writers[output_type] = VideoWriterPyAV(
                    video_path=save_path
                    / f"{self.train_step:06d}_{video_filename}.mp4",
                    fps=video_fps,
                )
            for output_type in output_types_for_gif:
                ...
                gif_writers[output_type] = VideoWriterPIL(
                    video_path=save_path
                    / f"{self.train_step:06d}_{video_filename}.gif",
                    fps=video_fps,
                )
            ...
            for output_type in output_types:
                ...
                if save_as_image:
                    dirname = (
                        ...
                        else output_type + "_" + image_name
                    )
                    (save_path / dirname).mkdir(exist_ok=True)
                    if output_type == "image":
                        image.save(
                            save_path / dirname / f"{self.train_step:06d}_{i:03d}.jpg",
                            quality=95,
                        )
                    else:
                        image.save(
                            save_path / dirname / f"{self.train_step:06d}_{i:03d}.png"
                        )
    ...
    def full_eval(self, fast: bool = False, rgb_only: bool = True):
        ...
        if not self.cfg.log.eval_only:
            ...
            dirname = (
                f"{self.cfg.data.test_h}x{self.cfg.data.test_w}_{self.cfg.prompt.scene}"
            )
        ...
        logger.info(
            f"Fully evaluating the last model, iteration #{self.train_step}, path: {str(final_renders_path)}"
        )
        self.evaluate(
        ...
        while self.train_step < self.max_step:
            ...
            if self.cfg.stage == "nerf":
                if (
                    ...
                    with torch.cuda.amp.autocast(enabled=self.cfg.optim.fp16):
                        self.model.update_extra_state(
                            random_sigmas=self.train_step == 0
                        )
            ...
            for optimizer in self.optimizers.values():
                ...
                if hasattr(optimizer, "update_learning_rate"):
                    optimizer.update_learning_rate(
                        spatial_scale=spatial_scale, iteration=self.train_step
                    )
            ...
            if self.time_to_snapshot:
                self.snapshot_image(
                    render_outputs["image"], filename="image", color_channel=3
                )
                self.snapshot_image(
                    visual_outputs["mask"], filename="mask", color_channel=1
                )

                if "cond_images" in data:
                    self.snapshot_condition(
                        visual_outputs["depth"], filename=self.controlnet_condition[0]
                    )

                logger.info(
                    f"train_step={self.train_step:06d}, spatial_scale={spatial_scale:.2f}"
                )
    ...
    def pretrain_forward(self, data):
        ...
        smpl_depth = np.nan_to_num(
            data["cond_images"][0], posinf=0.0, neginf=0.0
        )  # np.ndarray, [H, W]
        ...
        smpl_depth = torch.from_numpy(smpl_depth).to(
            device, dtype=torch.float
        )  # tensor, [H, W]
        smpl_depth = smpl_depth.unsqueeze(0).unsqueeze(0)  # tensor, [1, 1, H, W]

        if render_depth.shape[-2:] != smpl_depth.shape[-2:]:
            smpl_depth = torch.nn.functional.interpolate(
                smpl_depth, size=render_depth.shape[-2:], mode="bicubic"
            )
        ...
        while self.train_step < self.max_step:
            ...
            if self.time_to_snapshot:
                self.snapshot_image(
                    torch.cat(
                        [
                            render_outputs["image_fg"].detach(),
                            render_outputs["alpha"].detach(),
                        ],
                        dim=3,
                    ),
                    filename="image",
