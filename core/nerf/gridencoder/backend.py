...
def setup_windows_build_env():
    ...
    def find_windows_sdk_include():
        # Look directly in the Include directory
    ...
    def find_windows_sdk_lib():
        # Look directly in the Lib directory
    ...
    if os.name == "nt":  # Windows only
        ...
        # Add Visual Studio and Windows SDK include paths to environment
        include_paths = [
        ...
        # Verify paths exist
        for path in include_paths + lib_paths:
...
if os.path.exists(os.path.join(build_directory, "_gridencoder.pyd")):
    try:
        os.remove(os.path.join(build_directory, "_gridencoder.pyd"))
    except:
        pass
...
try:
    ...
    print(f"Failed to build _gridencoder extension: {str(e)}")
    raise
