...
def export_point_cloud(
    self, resolution=None, split_size=128, density_thresh=None
) -> BasicPointCloud:
    ...
    if density_thresh is None:
        if self.cuda_ray:
            density_thresh = (
                ...
                if np.greater(self.mean_density, 0)
...
def remove_points_inside_bboxes(
    point_cloud: BasicPointCloud, bboxes: Iterable[Iterable[Iterable[float]]]
) -> BasicPointCloud:
