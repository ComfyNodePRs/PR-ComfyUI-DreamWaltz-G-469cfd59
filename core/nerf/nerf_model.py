...
class BaseNeRFNetwork(NeRFRenderer):
    ...
    def forward(self, x, d, l=None, ratio=1, shading="albedo"):
        ...
        if shading == "albedo":
            ...
            if self.latent_mode:
                ...
                color = torch.cat(
                    [color, torch.zeros((color.shape[0], 1), device=color.device)],
                    axis=1,
                )
    ...
    def background(
        self, image: torch.Tensor, bg_mode: Optional[str], rays_d: torch.Tensor
    ):
        """
    ...
    def normal(
        self, x, normal_type: str = "finite_difference_laplacian", epsilon: float = 1e-3
    ):
        # x: [N, 3]
        if normal_type == "finite_difference_laplacian":
            ...
            normal = (
                -0.5
                * torch.stack(
                    [(dx_pos - dx_neg), (dy_pos - dy_neg), (dz_pos - dz_neg)], dim=-1
                )
                / epsilon
            )
        else:
    ...
    def build_optimizer(self, params, optimizer_type):
        if optimizer_type == "adam":
            ...
            optimizer = torch.optim.AdamW(
                params, betas=(0.9, 0.99), eps=1e-15, weight_decay=1e-3
            )  # weight_decay=1e-2
        elif optimizer_type == "adan":
            from core.optim.adan import Adan

            optimizer = Adan(
                params, eps=1e-8, weight_decay=2e-5, max_grad_norm=5.0, foreach=False
            )
    ...
    def build_scheduler(self, optimizer, lr_policy, diffusion=None):
        ...
        if lr_policy in ("none", "constant"):
            ...
            scheduler = make_scheduler(
                optimizer, lr_policy, step_size=int(self.max_step * 0.7)
            )
...
class _NeRFNetwork(BaseNeRFNetwork):
    def __init__(
        ...
        self.sigma_net = MLP(
            self.in_dim, 4 + additional_dim_size, hidden_dim, num_layers, bias=True
        )
    ...
    def get_optimizer(self, cfg: NeRFConfig, diffusion=None):
        ...
        scheduler = self.build_scheduler(
            optimizer, lr_policy=cfg.lr_policy, diffusion=diffusion
        )
...
class _NeRFNetwork_DualMLP(BaseNeRFNetwork):
    def __init__(
        ...
        self.albedo_net = MLP(
            self.in_dim, 3 + additional_dim_size, hidden_dim, num_layers, bias=True
        )
