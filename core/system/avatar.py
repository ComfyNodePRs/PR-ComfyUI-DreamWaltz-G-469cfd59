...
from core.gaussian.gaussian_optimizer import (
    OptimizationParams,
    GaussianOptimizer,
    build_optimizer,
)
...
from core.human.inverse_lbs import (
    LinearBlendSkinning,
    GeneralLinearBlendSkinning,
    RigidTransform,
)
from dw_utils.point_cloud import BasicPointCloud
from dw_utils.mesh import compute_normal
...
class _Avatar(GaussianModel):
    ...
    def initialize_positions(
        ...
        if point_cloud is not None:
            ...
            vertex_indices = [
                [i] * cfg.render.n_gaussians_per_vertex for i in range(n_vertices)
            ]
            ...
            raise NotImplementedError(
                "Initialization method 'mesh_triangle' is not implemented."
            )
    ...
    def initialize_colors(
        ...
        if point_cloud is not None:
            ...
            colors = (
                ((vn + 1) / 2.0).mean(dim=1, keepdim=True).repeat(1, 3)[vertex_indices]
            )
    ...
    def initialize_radiuses(
        ...
        knn_dists = knn_points(vertices[None], vertices[None], K=K + 1).dists[
            0, :, 1:
        ]  # [V, K-1]
        if use_sqrt:
    ...
    def initialize_scales(
        ...
        if gaussian_scale_init == "radius":
            ...
            scales = self.scale_inverse_activation(
                radiuses.expand(-1, 3) * cfg.render.init_scale_radius_rate
            )
        else:
            scales = self.scale_inverse_activation(
                torch.ones(n_points, 3) * cfg.render.init_scale
            )
        return scales

    def initialize_vertex_indices(
        self, positions: torch.Tensor, vertices: torch.Tensor
    ):
        return knn_points(
    ...
    def get_optimizer(self, cfg: TrainConfig) -> dict:
        ...
        optimizers["avatar"] = build_optimizer(
            model=self, cfg=cfg, optim_params=optim_params
        )
        return optimizers
...
class _AnimatableAvatar(_Avatar):
    ...
    def lbs_transform(
        self, gaussians: GaussianOutput, smpl_observed_inputs: dict
    ) -> GaussianOutput:
        ...
        cnl2obs_transform: RigidTransform = (
            canonical_transform.inverse().compose(observed_transform).squeeze(0)
        )
...
class VanillaAvatar(_AnimatableAvatar):
    def __init__(
        ...
        **kwargs,
        ...
        self.vertex_indices = self.initialize_vertex_indices(
            positions, self.canonical_vertices
        )

        # Initialize SH features
        colors = self.initialize_colors(
            cfg=cfg, n_points=self._n_points, point_cloud=point_cloud
        )
        self._sh_features_dc = nn.Parameter(
            RGB2SH(colors).unsqueeze(dim=1), requires_grad=learn_colors
        )
        self._sh_features_rest = nn.Parameter(
            torch.zeros(self._n_points, self.sh_levels**2 - 1, 3),
            requires_grad=learn_colors,
        )

        # Initialize opacities
        opacities = self.opacity_inverse_activation(
            cfg.render.init_opacity * torch.ones((self._n_points, 1))
        )
        ...
        quaternions = matrix_to_quaternion(
            torch.eye(3)[None].repeat(self._n_points, 1, 1)
        )  # [N, 4]
    ...
    def reset_by_state_dict(self, state_dict: Mapping[str, Any]):
        ...
        self.vertex_indices = self.initialize_vertex_indices(
            state_dict["_positions"], self.canonical_vertices
        )
...
class HashAvatar(_AnimatableAvatar):
    def __init__(
        ...
        **kwargs,
        ...
        self.nerf_scale_and_quaternion_net = MLP(
            nerf.in_dim, 7, dim_hidden=64, num_layers=3, bias=True
        )
    ...
    def nerf_forward(self, xyzs):
        # Forward
        enc = self.nerf_encoder(
            xyzs, bound=self.nerf_bound
        )  # xyzs: [N, 3] -> enc: [N, 32]
    ...
    def get_optimizer(self, cfg: TrainConfig):
        ...
        if hasattr(self, "nerf_scale_and_quaternion_net"):
            params += [
                {
                    "params": self.nerf_scale_and_quaternion_net.parameters(),
                    "lr": nerf_lr,
                },
            ]
        optimizers["nerf"] = torch.optim.Adam(
            params, betas=(0.9, 0.99), eps=1e-15, weight_decay=0
        )
        # Return
        return optimizers
...
class HashAvatarWithMesh(HashAvatar):
    def __init__(
        ...
        **kwargs,
        ...
        self.use_nerf_mesh_scales_and_quaternions = (
            cfg.render.use_nerf_mesh_scales_and_quaternions
        )
        ...
        n_points_on_surface = len(
            predefined_triangles
        ) * n_gaussians_per_triangle + len(predefined_vertex_indices)
        self._n_points_on_surface = n_points_on_surface

        bary_coords_per_triangle = self.prepare_mesh_binding_params(
            n_gaussians_per_triangle
        )  # [n_gaussians_per_triangle, 3]
        bary_coords = bary_coords_per_triangle.expand(
            len(predefined_triangles), -1, -1
        )  # [Nf, n_gaussians_per_triangle, 3]
        ...
        self._vertex_coords = nn.Parameter(
            vertex_coords, requires_grad=learn_vertex_coords
        )

        positions_on_surface = self.get_mesh_positions()
        vertex_indices_on_surface = knn_points(
            positions_on_surface[None], self.canonical_vertices[None], K=1
        ).idx[0, :, 0]
        ...
        if not self.use_nerf_mesh_scales_and_quaternions:
            ...
            self._scales_on_surface = nn.Parameter(
                scales_on_surface, requires_grad=self.learn_mesh_scales
            )
            # Initialize quaternions
            quaternions_on_surface = matrix_to_quaternion(
                torch.eye(3)[None].repeat(self._n_points_on_surface, 1, 1)
            )  # [N, 4]
            self._quaternions_on_surface = nn.Parameter(
                quaternions_on_surface, requires_grad=self.learn_mesh_quaternions
            )

        # Buffer Mechanism for mesh-binding 3D gaussians
        self.buffer_mesh_opacities_from_nerf = (
            cfg.render.buffer_mesh_opacities_from_nerf
        )
        self.buffer_mesh_scales_from_nerf = cfg.render.buffer_mesh_scales_from_nerf
        self.buffer_mesh_quaternions_from_nerf = (
            cfg.render.buffer_mesh_quaternions_from_nerf
        )
        ...
        self.requires_buffer_on_surface = (
            self.buffer_mesh_opacities_from_nerf
            or self.buffer_mesh_scales_from_nerf
            or self.buffer_mesh_quaternions_from_nerf
        )
    ...
    def prepare_mesh_binding_params(self, n_gaussians_per_triangle) -> torch.Tensor:
        ...
        if n_gaussians_per_triangle == 1:
            ...
            bary_coords_per_triangle = torch.rand(
                n_gaussians_per_triangle, 3, dtype=torch.float32
            )
    ...
    def reset_by_state_dict(self, state_dict: Mapping[str, Any]):
        ...
        for name in (
            "nerf_opacities_on_surface",
            "nerf_scales_on_surface",
            "nerf_quaternions_on_surface",
        ):
            if name in state_dict:
                self.register_buffer(name, state_dict[name])

    def get_mesh_positions(
        self, _bary_coords=None, _triangle_coords=None, _vertex_coords=None
    ):
        ...
        positions_on_triangles = torch.einsum(
            "fnv,fvc->fnc", bary_coords, _triangle_coords
        )
    ...
    def get_mesh_scales(self, return_means=False):
        if not return_means:
            ...
            return self.scale_activation(
                self._scales_on_surface.mean(dim=-1, keepdim=True).expand(-1, 3)
            )
    ...
    def buffer_mesh_attributes_from_nerf(self, positions_on_surface):
        with torch.no_grad():
            (
                opacities_on_surface,
                colors_on_surface,
                scales_on_surface,
                quaternions_on_surface,
    ...
    def nerf_mesh_forward(self, xyzs, disable_buffer: bool = False):
        ...
        if self.use_nerf_mesh_scales_and_quaternions:
            if (
                self.use_mesh_scales_buffer
                and self.use_mesh_quaternions_buffer
                ...
                if self.use_mesh_scales_buffer and not disable_buffer:
                    ...
                    scales = (
                        self.scale_activation(scales_quaternions[:, :3])
                        * self.init_scale
                    )
    ...
    def get_optimizer(self, cfg: TrainConfig):
        ...
        if not self.use_nerf_mesh_scales_and_quaternions:
            if self.learn_mesh_scales:
                l += [
                    {
                        "params": [self._scales_on_surface],
                        "lr": cfg.render.scaling_lr,
                        "name": "scales_on_surface",
                    }
                ]
            if self.learn_mesh_quaternions:
                l += [
                    {
                        "params": [self._quaternions_on_surface],
                        "lr": cfg.render.rotation_lr,
                        "name": "quaternions_on_surface",
                    }
                ]
        if len(l) > 0:
...
####################################################################################
def find_knn_vertex_indices(
    positions: torch.Tensor, vertices: torch.Tensor, K: int = 1
):
...
def find_nearest_triangles(points, vertices, triangles, device=None):
    ...
    squared_distances, triangle_indices, closest_points = (
        igl.point_mesh_squared_distance(points, vertices, triangles)
    )
    ...
    min_barycentric_indices = torch.argmin(
        barycentric_coords, dim=1, keepdim=True
    )  # [N, 1]
    nearest_vertex_indices = torch.gather(
        vertex_indices, 1, min_barycentric_indices
    ).squeeze(
        -1
    )  # [N,]
...
class LBSUtils:
    ...
    def initialize_lbs_weights(
        ...
        lbs_weights = lbs_model.lbs_weights[
            nearest_triangles_buffer["vertex_indices"]
        ]  # [N, 3, J]
        barycentric_coords = nearest_triangles_buffer["barycentric_coords"].to(
            lbs_weights.device
        )  # [N, 3]
        lbs_weights = torch.einsum(
            "nij,ni->nj", lbs_weights, barycentric_coords
        )  # [N, J]
        ...
        if smooth:
            ...
            mesh_dists = nearest_triangles_buffer["squared_distances"].to(
                lbs_weights.device
            )  # [N,]
            if use_sqrt:
                knn_dists = torch.sqrt(knn_dists)
            ...
            update_weights = dists_to_weights(
                mesh_dists, low=valid_dist_threshold
            ).unsqueeze(
                -1
            )  # [N, 1]
            # update_weights = None
            for _ in range(smooth_N):
                new_lbs_weights = torch.einsum(
                    "nk,nkj->nj", knn_weights, lbs_weights[knn_indices]
                )
                if update_weights is None:
                    ...
                    lbs_weights = (
                        1.0 - update_weights
                    ) * lbs_weights + update_weights * new_lbs_weights
...
class MeshBindingGaussianModel(nn.Module, LBSUtils):
    def __init__(
        ...
        bary_coords = self.initialize_bary_coords(
            self._n_points_per_triangle
        )  # [F, G_per_F, 3]
        self._bary_coords = nn.Parameter(bary_coords, requires_grad=learn_bary_coords)
        ...
        predefined_triangles = (
            canonical_triangles[predefined_triangle_indices].cpu().numpy()
        )
        ...
        points_to_triangles = (
            torch.arange(self._n_triangles)[..., None]
            .expand(-1, self._n_points_per_triangle)
            .reshape(-1)
        )  # [N,]
        ...
        self._scales = nn.Parameter(
            torch.ones(self._n_points, 3) * init_scale_ratio, requires_grad=learn_scales
        )
    ...
    def initialize_bary_coords(self, n_gaussians_per_triangle) -> torch.Tensor:
        ...
        if n_gaussians_per_triangle == 1:
            ...
            bary_coords_per_triangle = self.bary_coord_activation(
                bary_coords_per_triangle
            )
    ...
    def get_positions(self, vertex_coords=None, bary_coords=None):
        if bary_coords is None:
            bary_coords = self.bary_coord_activation(
                self._bary_coords
            )  # [F, G_per_F, 3]
    ...
    def get_scales_and_quaternions(self, vertex_coords, positions, eps=1e-9):
        ...
        v0 = point_normals / (
            torch.linalg.vector_norm(point_normals, dim=-1, keepdim=True) + eps
        )

        reference = torch.tensor(
            (1.0, 0.0, 0.0), dtype=torch.float32, device=positions.device
        ).expand_as(p0)
        ...
        s1 = (
            dot(p1 - p0, v1).abs() + dot(p2 - p0, v1).abs() + dot(p3 - p0, v1).abs()
        ) / self._n_points_per_triangle
        s2 = (
            dot(p1 - p0, v2).abs() + dot(p2 - p0, v2).abs() + dot(p3 - p0, v2).abs()
        ) / self._n_points_per_triangle
...
class DreamWaltzG(_AnimatableAvatar, LBSUtils):
    def __init__(
        ...
        **kwargs,
        ...
        self.render_mesh_binding_3d_gaussians_only = (
            cfg.render.render_mesh_binding_3d_gaussians_only
        )
        self.render_unconstrained_3d_gaussians_only = (
            cfg.render.render_unconstrained_3d_gaussians_only
        )
        ...
        if self.use_nerf_encoded_position:
            self.nerf_scale_and_quaternion_net = DeformNetwork(
                xyz_input_ch=32, D=4, W=64, residual=False
            )
        else:
            self.nerf_scale_and_quaternion_net = DeformNetwork(
                xyz_input_ch=None, D=4, W=64, residual=False
            )
        ...
        self._betas = nn.Parameter(
            self.lbs_model.betas.data.clone(), requires_grad=self.learn_betas
        )
        ...
        # Initialize scales
        scales = self.scale_inverse_activation(
            torch.ones(self._n_points, 3) * self.init_scale
        )
        self._scales = nn.Parameter(scales, requires_grad=learn_scales)
    ...
    def reset_by_state_dict(self, state_dict: Mapping[str, Any]):
        ...
        if hasattr(self, "_lbs_weights"):
            if "_lbs_weights" not in state_dict:
                ...
                self._lbs_weights = nn.Parameter(
                    lbs_weights, requires_grad=self.learn_lbs_weights
                )
            else:
    ...
    def static_mlp_forward(
        self, enc: torch.Tensor, fix_opacities: bool = False
    ...
    def dynamic_mlp_forward(
        self, enc: torch.Tensor, body_pose: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        nerf_offsets, nerf_scales, nerf_quaternions = (
            self.nerf_scale_and_quaternion_net(enc, body_pose)
        )
    ...
    def get_unconstrained_gaussians(
        ...
        # Get colors and opacities
        enc = self.nerf_encoder(
            canonical_positions, bound=self.nerf_bound
        )  # positions: [N, 3] -> enc: [N, 32]
        ...
        if self.use_nerf_encoded_position:
            offsets, scales, quaternions = self.dynamic_mlp_forward(
                enc, body_pose=body_pose
            )
        else:
            offsets, scales, quaternions = self.dynamic_mlp_forward(
                positions.detach(), body_pose=body_pose
            )
    ...
    def get_constrained_gaussians(
        ...
        if use_nerf:
            enc = self.nerf_encoder(
                positions, bound=self.nerf_bound
            )  # positions: [N, 3] -> enc: [N, 32]
            colors, opacities = self.static_mlp_forward(
                enc, fix_opacities=fix_opacities
            )
            if mean_color:
                n_points = colors.size(0)
                mean_colors = torch.mean(colors, dim=0, keepdim=True).expand(
                    n_points, -1
                )
                colors = (
                    mean_color_rate * mean_colors + (1.0 - mean_color_rate) * colors
                )
        else:
            assert 0
    ...
    def get_opacities(self, return_ones: bool = False) -> torch.Tensor:
        ...
        _, _, canonical_transforms = self.lbs_model.forward(
            **self.smpl_canonical_inputs
        )
    ...
    def inverse_lbs_transform(
        ...
        if 0:  # Wrong
            ...
            positions = inverse_joint_transform.transform_points(
                positions, weights=lbs_weights
            )
        else:  # Correct
            forward_joint_transform = (
                RigidTransform.compose(
                    transforms["J_pose_rigid"],
                    transforms["G_transl_offset"],
                )
            ...
            positions = RigidTransform._inverse_transform_points(
                positions, R=R_forward, T=T_forward
            )

        # Pose offsets
        if self.use_vertex_pose_offsets:
            inverse_pose_transform: RigidTransform = (
                transforms["V_pose_offset"].inverse().squeeze(0)
            )
            positions = inverse_pose_transform.transform_points(
                positions, indices=vertex_indices
            )

        # Shape offsets
        if self.use_vertex_shape_offsets:
            inverse_shape_transform: RigidTransform = (
                transforms["V_shape_offset"].inverse().squeeze(0)
            )
            positions = inverse_shape_transform.transform_points(
                positions, indices=vertex_indices
            )
        elif self.use_joint_shape_offsets:
            inverse_shape_transform: RigidTransform = (
                transforms["J_shape_offset"].inverse().squeeze(0)
            )
            positions = inverse_shape_transform.transform_points(
                positions, weights=lbs_weights
            )
    ...
    def lbs_transform(
        ...
        # shape offsets
        if self.use_vertex_shape_offsets:
            vertex_shape_transform: RigidTransform = transforms[
                "V_shape_offset"
            ].squeeze(0)
            positions = vertex_shape_transform.transform_points(
                positions, indices=vertex_indices
            )
        elif self.use_joint_shape_offsets:
            joint_shape_transform: RigidTransform = transforms[
                "J_shape_offset"
            ].squeeze(0)
            positions = joint_shape_transform.transform_points(
                positions, weights=lbs_weights
            )
        # pose offsets
        if self.use_vertex_pose_offsets:
            vertex_pose_transform: RigidTransform = transforms["V_pose_offset"].squeeze(
                0
            )
            positions = vertex_pose_transform.transform_points(
                positions, indices=vertex_indices
            )
        # pose transforms
    ...
    def non_rigid_transform(self, gaussians: GaussianOutput) -> GaussianOutput:
        if self.use_non_rigid_offsets:
            gaussians.positions = (
                gaussians.positions + gaussians.offsets * self.init_offset
            )
        ...
        if self.use_non_rigid_scales:
            if self.learn_scale:
                if self.non_rigid_rotation_mode == "add":
                    gaussians.scales = (
                        self.get_scales() + gaussians.scales * self.init_scale
                    )
                else:
                    gaussians.scales = self.get_scales() * (
                        1.0 + gaussians.scales * self.init_scale
                    )
            else:
                gaussians.scales = (
                    self.scale_activation(gaussians.scales) * self.init_scale
                ).clamp_max_(max=self.max_scale)
        else:
            if self.learn_scale:
                gaussians.scales = self.get_scales()
            else:
            ...
            if self.learn_quaternions:
                if self.non_rigid_rotation_mode == "add":
                    gaussians.quaternions = (
                        self.get_quaternions() + gaussians.quaternions
                    )
                else:
                    new_quaternions = self.rotation_activation(gaussians.quaternions)
                    gaussians.quaternions = quaternion_multiply(
                        new_quaternions, self.get_quaternions()
                    )
            else:
    ...
    def animate(self, smpl_observed_inputs: Optional[dict] = None) -> GaussianOutput:
        ...
        # Get LBS Transform from smpl-canonical to canonical space
        _, canonical_vertex_transform, canonical_transforms = self.lbs_model.forward(
            **smpl_canonical_inputs
        )

        # Get LBS Transform from smpl-canonical to observed space
        _, observed_vertex_transform, observed_transforms = self.lbs_model.forward(
            **smpl_observed_inputs
        )
        ...
        if self.learn_betas:
            _, canonical_vertex_transform_with_betas, _ = self.lbs_model.forward(
                **smpl_canonical_inputs, extra_betas=self._betas
            )
            _, observed_vertex_transform_with_betas, _ = self.lbs_model.forward(
                **smpl_observed_inputs, extra_betas=self._betas
            )
        ...
        for body_part, gaussian_model in self.mesh_binding_gaussians.items():
            ...
            if (body_part == "hands" and self.learn_hand_betas) or (
                ...
                cnl_vertex_transform: RigidTransform = (
                    canonical_vertex_transform_with_betas.squeeze(0)
                )
                obs_vertex_transform: RigidTransform = (
                    observed_vertex_transform_with_betas.squeeze(0)
                )
            else:
                cnl_vertex_transform: RigidTransform = (
                    canonical_vertex_transform.squeeze(0)
                )
                obs_vertex_transform: RigidTransform = (
                    observed_vertex_transform.squeeze(0)
                )

            # Mesh-independent Forward
            canonical_vertex_coords = cnl_vertex_transform.transform_points(
                vertex_coords, indices=gaussian_model.predefined_vertex_indices
            )
            canonical_positions = gaussian_model.get_positions(
                vertex_coords=canonical_vertex_coords
            )
            ...
            # LBS Transform
            observed_vertex_coords = obs_vertex_transform.transform_points(
                vertex_coords, indices=gaussian_model.predefined_vertex_indices
            )
            # Mesh-dependent Forward
            constrained_gaussians.positions = gaussian_model.get_positions(
                vertex_coords=observed_vertex_coords
            )
            constrained_gaussians.scales, constrained_gaussians.quaternions = (
                gaussian_model.get_scales_and_quaternions(
                    vertex_coords=observed_vertex_coords,
    ...
    def get_optimizer(self, cfg: TrainConfig):
        ...
        gaussian_optimizer = build_optimizer(
            model=self, cfg=cfg, optim_params=optim_params
        )
        ...
        if self.lbs_model is not None:
            ...
            if self.learn_lbs_weights:
                params_list.append(
                    {"params": [self._lbs_weights], "lr": cfg.render.lbs_lr}
                )
            if self.learn_betas:
        ...
        for model_name, gaussian_model in self.mesh_binding_gaussians.items():
            gaussian_model: MeshBindingGaussianModel
            optimizers.update(
                gaussian_model.get_optimizer(
                    cfg=cfg, optimizer_name="mesh_" + model_name
                )
            )
        # Return
        return optimizers
...
####################################################################################
def build_gaussian_avatar(
    cfg: TrainConfig, smpl_prompt: SMPLPrompt, point_cloud=None, nerf=None
):
    ...
    if gs_type == "vanilla":
        ...
        for part_name in body_parts:
            predefined_vids, predefined_fids = smpl_model.get_semantic_indices(
                part_name
            )
