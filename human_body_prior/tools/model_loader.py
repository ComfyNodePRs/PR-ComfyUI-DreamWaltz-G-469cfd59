...
def exprdir2model(expr_dir, model_cfg_override: dict = None):
    """
    Load model configuration from experiment directory, works on both Windows and Linux.

    Args:
        expr_dir: Path to experiment directory
        model_cfg_override: Optional dictionary of configuration overrides

    Returns:
        tuple: (model_cfg: OmegaConf configuration object, trained_weights_fname: Path to weights file)
    """
    ...
    # Use Path for snapshot directory
    model_snapshots_dir = expr_dir / "snapshots"

    # Use Path.glob() for checkpoint files
    available_ckpts = sorted(
        model_snapshots_dir.glob("*.ckpt"), key=lambda x: x.stat().st_mtime
    )
    ...
    # Get parent directories of checkpoint file
    config_search_dir = trained_weights_fname.parent.parent

    # Search for yaml files in the parent directory
    yaml_files = list(config_search_dir.glob("*.yaml"))

    if not yaml_files:
        # Try current directory if not found in parent
        yaml_files = list(Path().glob("*.yaml"))

    if not yaml_files:
        raise ValueError(f"No yaml configuration file found in {config_search_dir}")

    model_cfg_fname = yaml_files[0]
    model_cfg = OmegaConf.load(str(model_cfg_fname))
